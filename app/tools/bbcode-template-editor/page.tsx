...
export default function BBCodeTemplateEditor() {
  ...
  try {
    ...
    while ((match = fieldRegex.exec(bbcodeContent)) !== null) {
      ...
      const field: TemplateField = {
        ...
        value: '',
        defaultValue: '',
        localStorage: false // Individual field localStorage control - starts as false
      };
  ...
  const getDefaultBBCodeFormat = (type: string): BBCodeFormat => {
    switch (type) {
      ...
      case 'checkbox':
        return { prefix: '', suffix: '' };
      case 'list':
        return { prefix: '[list]', suffix: '[/list]' };
      default:
        return { prefix: '', suffix: '' };
    }
  };
  ...
  const downloadTemplate = () => {
    ...
    try {
      // Create a copy of the template with the latest field values from localStorage
      const templateToDownload = { ...template };
      
      // Update field values from localStorage if available
      templateToDownload.fields = template.fields.map(field => {
      ...
      const jsonString = JSON.stringify(templateToDownload, null, 2);
...
{{list:items}}`}
          ...
          {/* Preview Panel - localStorage functionality is handled inside TemplatePreview */}
        <TemplatePreview
          template={template}
